...
class GuestController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        if (!$location) {
            ...
            // Aggiunge anche gli sponsor e ordina gli appartamenti con sponsor per primi
            $apartments = $apartments->with('extraServices', 'sponsors')
                ->orderByRaw('(CASE WHEN EXISTS (SELECT 1 FROM apartment_sponsor WHERE apartment_sponsor.apartment_id = apartments.id) THEN 1 ELSE 0 END) DESC')
                ->get();
        ...
        // Aggiunge gli sponsor e ordina gli appartamenti con sponsor per primi
        $apartments = $apartments->with('extraServices', 'sponsors')
            ->orderByRaw('(CASE WHEN EXISTS (SELECT 1 FROM apartment_sponsor WHERE apartment_sponsor.apartment_id = apartments.id) THEN 1 ELSE 0 END) DESC')
            ->get();
